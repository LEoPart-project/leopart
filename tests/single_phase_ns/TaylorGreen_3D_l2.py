...
from dolfin import (SubDomain, Constant, Expression, FunctionSpace, VectorElement,
                    ...
                    Point, as_vector, assign, XDMFFile, BoxMesh, project, curl,
...
class PeriodicBoundary(SubDomain):
    ...
    def inside(self, x, on_boundary):
        (xmin, xmax, ymin, ymax, zmin, zmax) = (self.xmin, self.xmax, self.ymin, self.ymax,
                                                self.zmin, self.zmax)
...
if comm.rank == 0:
    ...
    with open(output_table, "w") as write_file:
        write_file.write("%-12s %-20s %-20s %-20s \n" % ("Time level", "Kinetic energy", "Vorticity", "Momentum"))
...
# Compute some physical quantities
...
while step < num_steps:
    ...
    if comm.rank == 0:
        ...
        with open(output_table, "a") as write_file:
            write_file.write("%-12.5g %-20.6g %-20.6g %-20.6e \n" % (float(t), float(e_kin), float(enstrophy), float(momentum)))
