...
StokesStaticCondensation::StokesStaticCondensation(const Mesh& mesh,
    ...
    : mesh(&mesh), A(&A), B(&B), G(&G), Q(&Q), S(&S),
                   ...
                   Be_list(mesh.num_cells()),
                   Qe_list(mesh.num_cells()), mpi_comm(mesh.mpi_comm())
{
...
//-----------------------------------------------------------------------------
StokesStaticCondensation::StokesStaticCondensation(const Mesh& mesh,  const Form& A, const Form& G, const Form& B,
                                            const Form& Q, const Form& S,
    ...
    : StokesStaticCondensation::StokesStaticCondensation(mesh, A, G, B, Q, S)
{
    ...
    // TODO: assemble systems such that symmetry is preserved! see SystemAssembler.cpp
    // TODO: Check that B is square, rectangular not yet implemented
}
//-----------------------------------------------------------------------------
StokesStaticCondensation::StokesStaticCondensation(const Mesh& mesh,  const Form& A,  const Form& G,
...
StokesStaticCondensation::StokesStaticCondensation(const Mesh& mesh,  const Form& A,  const Form& G,
                                            ...
                                            const Form& Q,  const Form& S,
                                            std::vector<std::shared_ptr<const DirichletBC>> bcs)
    : StokesStaticCondensation::StokesStaticCondensation(mesh, A, G, GT, B, Q, S)
{
    this->bcs = bcs;
}
...
void StokesStaticCondensation::assemble_global_lhs()
{
    ...
    for( CellIterator cell(*(this->mesh)); !cell.end(); ++cell){
        std::size_t nrowsA, ncolsA,
                    nrowsB, ncolsB,
                    nrowsG, ncolsG;
//        TO BE DEPRECATED
//        ArrayView<const dolfin::la_index> cdof_rowsA, cdof_colsA,
//                                          cdof_rowsB, cdof_colsB,
//                                          cdof_rowsG, cdof_colsG;

//        // Get local tensor info
//        local_tensor_info(*(this->A), *cell, &nrowsA, cdof_rowsA,
//                                             &ncolsA, cdof_colsA);
//        local_tensor_info(*(this->B), *cell, &nrowsB, cdof_rowsB,
//                                             &ncolsB, cdof_colsB);
//        local_tensor_info(*(this->G), *cell, &nrowsG, cdof_rowsG,
//                                             &ncolsG, cdof_colsG);
        ...
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> A_e, B_e, G_e;
        ...
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> invA_e = A_e.inverse();
...
//            ArrayView<const dolfin::la_index> cdof_rowsGT, cdof_colsGT;
//            local_tensor_info(*(this->GT), *cell, &nrowsGT, cdof_rowsGT,
//                                                  &ncolsGT, cdof_colsGT);

            std::tie(nrowsGT, ncolsGT) = FormUtils::local_tensor_size(*(this->GT), *cell);

            Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> GT_e;
        ...
        auto cdof_rowsB = this->B->function_space(0)->dofmap()->cell_dofs(cell->index());
        auto cdof_colsB = this->B->function_space(1)->dofmap()->cell_dofs(cell->index());
...
void StokesStaticCondensation::assemble_global_rhs()
{
    ...
    if ( invAe_list[0].size() == 0 || Ge_list[0].size() == 0 )
        dolfin_error("StokesStaticCondensation::assemble_global_rhs", "assemble global RHS vector",
...
//        local_tensor_info(*(this->Q), *cell, &nrowsQ, cdof_rowsQ, &ncolsQ, cdof_colsQ);
//        local_tensor_info(*(this->S), *cell, &nrowsS, cdof_rowsS, &ncolsS, cdof_colsS);
        ...
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> Q_e, S_e;
        //Eigen::Matrix<double, Eigen::Dynamic, 1> Q_etest;
        ...
        if(assume_symmetric){
            RHS_e = Ge_list[cell->index()].transpose() * invAe_list[cell->index()] * Q_e - S_e;
        }else{
            RHS_e = GTe_list[cell->index()] * invAe_list[cell->index()] * Q_e - S_e;
        }

        // Alternatively, use this->B?!
        auto cdof_rowsS = this->S->function_space(0)->dofmap()->cell_dofs(cell->index());
...
void StokesStaticCondensation::assemble_global_system(bool assemble_lhs){
    ...
    // useful when using iterative solver (hopefully...), but note that it requires
    // the global system to be reassembled every time

    // You may skip repeated assemblage of A_e (when desired)
    if(assemble_lhs) A_g.zero();
    f_g.zero();
    ...
    for( CellIterator cell(*(this->mesh)); !cell.end(); ++cell){
        ...
        std::size_t nrowsA, ncolsA, nrowsB, ncolsB,
                    nrowsG, ncolsG;

        // TODO: TO BE DEPRECATED
        //nrowsQ, ncolsQ, nrowsS, ncolsS;
//        ArrayView<const dolfin::la_index> cdof_rowsA, cdof_colsA,
//                                          cdof_rowsB, cdof_colsB,
//                                          cdof_rowsG, cdof_colsG;
        // TODO: TO BE DEPRECATED
                                          //cdof_rowsQ, cdof_colsQ,
                                          //cdof_rowsS, cdof_colsS;

        // Get local tensor info LHS
//        local_tensor_info(*(this->A), *cell, &nrowsA, cdof_rowsA,
//                                             &ncolsA, cdof_colsA);
//        local_tensor_info(*(this->B), *cell, &nrowsB, cdof_rowsB,
//                                             &ncolsB, cdof_colsB);
//        local_tensor_info(*(this->G), *cell, &nrowsG, cdof_rowsG,
//                                             &ncolsG, cdof_colsG);
        // Get local tensor info RHS (TO BE DEPRECATED)
        //local_tensor_info(*(this->Q), *cell, &nrowsQ, cdof_rowsQ, &ncolsQ, cdof_colsQ);
        //local_tensor_info(*(this->S), *cell, &nrowsS, cdof_rowsS, &ncolsS, cdof_colsS);
...
//                                                      &ncolsGT, cdof_colsGT);
        ...
        // TODO: TO BE DEPRECATED
        //FormUtils::FormUtils::local_assembler(Q_e, *(this->Q), *cell, nrowsQ, ncolsQ);
        //FormUtils::local_assembler(S_e, *(this->S), *cell, nrowsS, ncolsS);
        ...
        if(assume_symmetric){
            LHS_e = Ge_list[cell->index()].transpose() * invAe_list[cell->index()] * Ge_list[cell->index()]
                    - Be_list[cell->index()];
            RHS_e = Ge_list[cell->index()].transpose() * invAe_list[cell->index()] * Q_e - S_e;
        }else{
            LHS_e = GTe_list[cell->index()] * invAe_list[cell->index()] * Ge_list[cell->index()]
                    - Be_list[cell->index()];
            RHS_e = GTe_list[cell->index()] * invAe_list[cell->index()] * Q_e - S_e;
        }
        ...
        if (active_bcs){
            apply_boundary_symmetric(LHS_e, RHS_e, cdof_rowsB, cdof_colsB,
...
//-----------------------------------------------------------------------------
void StokesStaticCondensation::solve_problem(Function& Uglobal, Function& Ulocal,
...
void StokesStaticCondensation::apply_boundary(DirichletBC& DBC){
    ...
    std::cout<<"Matrix symmetry after apply_boundary? "<<A_g.is_symmetric(1E-6)<<std::endl;
}

//void StokesStaticCondensation::local_tensor_info(const Form& a, const Cell& cell,
...
// TO BE DEPRECATED
//void StokesStaticCondensation::local_assembler(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> &A_e,
...
//}
//-----------------------------------------------------------------------------
void StokesStaticCondensation::backsubtitute(const Function &Uglobal, Function &Ulocal){
    for( CellIterator cell(*(this->mesh)); !cell.end(); ++cell){
        std::size_t nrowsQ, ncolsQ, nrowsS, ncolsS;
//        ArrayView<const dolfin::la_index> cdof_rowsQ, cdof_colsQ, cdof_rowsS, cdof_colsS;
//        local_tensor_info(*(this->Q), *cell, &nrowsQ, cdof_rowsQ, &ncolsQ, cdof_colsQ);
//        local_tensor_info(*(this->S), *cell, &nrowsS, cdof_rowsS, &ncolsS, cdof_colsS);
        ...
        auto cdof_rowsS = this->S->function_space(0)->dofmap()->cell_dofs(cell->index());
        auto cdof_rowsQ = this->Q->function_space(0)->dofmap()->cell_dofs(cell->index());
        ...
        Ulocal_e = invAe_list[cell->index()] * (Qe_list[cell->index()] - Ge_list[cell->index()] * Uglobal_e );
        Ulocal.vector()->set_local(Ulocal_e.data(), Ulocal_e.size(), cdof_rowsQ.data());
    }
...
//-----------------------------------------------------------------------------
// TODO: migrated to FormUtils
void StokesStaticCondensation::apply_boundary_symmetric(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& LHS_e,
                                                         Eigen::Matrix<double, Eigen::Dynamic, 1>& RHS_e,
