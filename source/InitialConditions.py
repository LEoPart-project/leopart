...
class GaussianPulse(Expression):
    """
    ...
    def eval(self, value, x):
        ...
        #value[0] = self.height * np.exp(-(pow(x[0] - xc - U[0]* x[1] * self.t,2)+pow(x[1]-yc - U[1]* x[0] * self.t,2))/(2*pow(self.sigma,2)))
        
        value[0] = self.height * np.exp( -(pow(x[0]*np.cos(U[0]*t) + x[1]*np.sin(U[1]*t) - xc,2)+pow(-x[0]*np.sin(U[0]*t) + x[1]*np.cos(U[1]*t)-yc,2))/(2*pow(sigma,2) ))
        
    def value_shape(self):
        return ()

class SineHump(Expression):
